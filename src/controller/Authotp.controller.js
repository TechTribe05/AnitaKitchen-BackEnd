...
try {
    transporter = nodemailer.createTransport({
        ...
        // }, 
        connectionTimeout: 60000,    //60 second
        greetingTimeout: 30000,    //30 second 
        socketTimeout: 60000,  //60 second
    });
    console.log("✅ Email transporter created successfully");
    console.log(`✅ Using email: ${process.env.EMAIL}`);

    //Lets test the connection setup
...
function storedOTP(email, otp) {
    const expiryTime = Date.now() +10 * 60 * 1000;   //10 min expiration
...
async function sendOTP(req, res){
    try{
        ...
        res.status(200).json({success: true,  message: "OTP sent successfully", expiryTime: expiryTime })
    }catch(error) {
...
async function verifyOTP(req, res) {
    try {
        ...
        if(Date.now() > storedData.expiryTime) {
            ...
            return res.status(400).json({sucess: false, error: "OTP has expired", });
        }
